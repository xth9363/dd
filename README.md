...
如事务1200修改v的值为6，并提交了事务，此时活跃事务仍是[123,234,900] ，
但是1200已经结束，且不存在于活跃事务中，
1200的修改值6能被读到吗？

不能，如果读到6，就跟前两次读到的4不同，又出现了"不可重复读"的问题
...
如果trx_id_min <= trx_id <= trx_id_max, 那么表明该行记录所在事务在本次新事务创建的时候处于活动状态，
从trx_id_min到trx_id_max进行遍历，如果trx_id等于他们之中的某个事务id的话，那么不可见。
```
...
2.v=5 trx_id 900 在活跃事务中，不读

3.v=4 trx_id 600 123~900之间， 且不在活跃事务中，读取
